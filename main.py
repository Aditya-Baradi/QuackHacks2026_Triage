...
def _error_file_response(message: str, status_code: int = 500) -> FileResponse:
    TMP_DIR.mkdir(parents=True, exist_ok=True)
    error_path = TMP_DIR / f"error_{uuid4().hex}.txt"
    _write_text_file(error_path, f"ERROR:\n{message}\n")
    return FileResponse(
        path=str(error_path),
        media_type="text/plain",
        filename=error_path.name,
...
def _parse_bool_form(value: str) -> bool:
    ...
    if normalized == "false":
        return False
    raise ValueError("was_repeat must be 'true' or 'false'")
...
async def _validation_exception_handler(_, exc: RequestValidationError) -> FileResponse:
    return _error_file_response(f"Validation error: {exc}", status_code=422)


@app.exception_handler(HTTPException)
async def _http_exception_handler(_, exc: HTTPException) -> FileResponse:
    return _error_file_response(str(exc.detail), status_code=exc.status_code)


@app.exception_handler(StarletteHTTPException)
async def _starlette_http_exception_handler(_, exc: StarletteHTTPException) -> FileResponse:
    return _error_file_response(str(exc.detail), status_code=exc.status_code)


@app.exception_handler(Exception)
async def _unhandled_exception_handler(_, exc: Exception) -> FileResponse:
    return _error_file_response(str(exc), status_code=500)


def _is_wav_upload(upload: UploadFile) -> bool:
    name = (upload.filename or "").lower()
    ctype = (upload.content_type or "").lower()
    return name.endswith(".wav") or ctype in ("audio/wav", "audio/x-wav", "audio/wave")


def _is_webm_upload(upload: UploadFile) -> bool:
    name = (upload.filename or "").lower()
    ctype = (upload.content_type or "").lower()
    return name.endswith(".webm") or "webm" in ctype
...
async def analyze_audio(audio: UploadFile = File(...)) -> FileResponse:
    try:
        # Auto-identifiers (so frontend doesn't need the big panel)
        patient_id = "auto_patient"
        session_id = f"session_{datetime.now().strftime('%Y%m%d')}"
        question_text = ""
        was_repeat = "false"
        repeat_reason = ""
        repeat_requested = _parse_bool_form(was_repeat)
        ...
        # Save the upload with a meaningful suffix
        if _is_wav_upload(audio):
            input_path = TMP_DIR / f"{upload_id}.wav"
        elif _is_webm_upload(audio):
            input_path = TMP_DIR / f"{upload_id}.webm"
        else:
            input_path = TMP_DIR / f"{upload_id}_{_sanitize_segment(audio.filename or 'upload')}"

        with input_path.open("wb") as destination:
            shutil.copyfileobj(audio.file, destination)

        # Ensure we end up with a WAV for downstream steps
        wav_path = TMP_DIR / f"{upload_id}.wav"
        if input_path.suffix.lower() == ".wav":
            wav_path = input_path
        elif input_path.suffix.lower() == ".webm":
            convert_webm_to_wav(input_path, wav_path)
        else:
            raise HTTPException(
                status_code=400,
                detail="Unsupported audio format. Please upload a .wav (preferred) or .webm file.",
            )
        ...
        # Always write transcript-only file (2-file requirement)
        ...
        # The text-based disorientation trigger (place/time/memory confusion)
        ...
        # Keep the original flag name (now aligned to text trigger)
        ...
        # Extra audio supports for cognitive concern (used in new disorientation_possible logic)
        very_slow_speech = (
        ...
        # -----------------------------
        # NEW disorientation_possible logic (clinically more realistic)
        # disorientation_possible =
        #   text_disorientation_trigger
        #   OR
        #   (
        #     question_confusion
        #     AND (possible_slurred_speech OR very_slow_speech OR long_pauses)
        #   )
        # -----------------------------
        ...
        # "Max voice bonus" = loud + fast (energy/urgency, not necessarily shouting)
        max_voice_bonus = (
            rms_p95 is not None
            and speech_rate_est is not None
            and duration_seconds is not None
        ...
        # -----------------------------
        # AFFECT PRIORITY / CONFLICT RULES
        # -----------------------------
        affect_candidates = {
        ...
        repeat_value = "YES" if repeat_requested else "NO"
...
@app.post("/log-question")
async def log_question(payload: LogQuestionRequest) -> FileResponse:
    try:
